...
def load_legislator_mappings(csv_file):
    """
    ...
    Returns:
        dict: A dictionary mapping cleaned names to BioGuide IDs.
    """
...
def update_person_with_bioguide(person, name_mappings):
    """
    Update a person's entry with bioguide_id(s) if match(es) found.

    Args:
        person (dict): The person's entry in the JSON data.
        name_mappings (dict): A dictionary mapping cleaned names to BioGuide IDs.
    
    Returns:
        bool: True if a match was found and updated, False otherwise.
    """
    if (not person.get('member_name')) and \
       (not person.get('staff_name')):
        return False
...
def update_json_with_bioguides(data, name_mappings):
    """
    Recursively update all people in the JSON with BioGuide IDs.
    
    Args:
        data (dict): The JSON data to update.
        name_mappings (dict): A dictionary mapping cleaned names to BioGuide IDs.

    Returns:
        tuple: A tuple containing the updated data and statistics (showing matched, unmatched, multiple matches, etc).
    """
        ...
        if 'members' in committee:
            for member in committee['members']:
                ...
                if update_person_with_bioguide(member, name_mappings):
                    ...
                    stats['unmatched_people'].append(f"Committee '{committee['committee_name']}' member: {member['member_name']}")
        ...
        if 'staff' in committee:
            for staff in committee['staff']:
                ...
                if update_person_with_bioguide(staff, name_mappings):
                    ...
                    stats['unmatched_people'].append(f"Committee '{committee['committee_name']}' staff: {staff['staff_name']}")
            ...
            for subcommittee in committee['subcommittees']:
                ...
                if 'subcommittee_members' in subcommittee:
                    for member in subcommittee['subcommittee_members']:
                        stats['total_people'] += 1
                        member_name = member['member_name'] if 'member_name' in member else member['staff_name']
                        if update_person_with_bioguide(member, name_mappings):
                            stats['matched_people'] += 1
                            if isinstance(member.get('bioguide_id'), list):
                                stats['multiple_matches'] += 1
