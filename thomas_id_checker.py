...
def analyze_committee_coverage(json_file):
    """
    Analyze the coverage of Thomas IDs in the committee JSON file.

    Args:
        json_file (str): Path to the JSON file containing committee data.

    Returns:
        Tuple[Dict[str, int], List[str], List[Tuple[str, str]]]: A tuple containing the statistics for committee and subcommittee mapping,
            a list of unmapped committees, and a list of unmapped subcommittees (grouped by parent committee
    """
    try:
        ...
        unmapped_committees = []
        unmapped_subcommittees = []  # Will store tuples of (parent_committee, subcommittee)
            ...
            if 'subcommittees' in committee:
                ...
                for subcommittee in subcommittees:
                    ...
                    if 'thomas_id' in subcommittee:
                        ...
                        unmapped_subcommittees.append((committee_name, subcommittee_name))
        
        # Calculate percentages
        committee_percentage = (stats['mapped_committees'] / stats['total_committees'] * 100) if stats['total_committees'] > 0 else 0
        subcommittee_percentage = (stats['mapped_subcommittees'] / stats['total_subcommittees'] * 100) if stats['total_subcommittees'] > 0 else 0
        
        # Print results
        print("\nCommittee Mapping Statistics:")
        print("=" * 80)
        print(f"Main Committees:")
        ...
        if unmapped_subcommittees:
            ...
            current_parent = None
            for parent, subcommittee in sorted(unmapped_subcommittees, key=lambda x: x[0]):
...
def main():
    ...
    stats, unmapped_committees, unmapped_subcommittees = analyze_committee_coverage(input_file)
