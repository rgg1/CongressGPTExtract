"""
Extracts department/cabinet/office of the president, member names, and roles from the given
congressional directory text files using OpenAI's API. The extracted information is returned in JSON
format.
"""
...
class DepartmentsJsonSchema(BaseModel):
    cabinet: list[Member]
    ...
    try:
        response = client.chat.completions.create(
            ...
            response_format={
                ...
                "json_schema": {
                    "name": "_",
                    "schema": DepartmentsJsonSchema.model_json_schema(),
                },
            },
            temperature=0.3,
...
def chunk_departments_text(text: str, max_chunk_size: int =15000) -> list[str]:
    """
    Splits the given text into chunks of maximum size `max_chunk_size` while preserving the
    organizational structure of the text. The function identifies organizational headers and
    ensures that they are not split across chunks.
    ...
    def is_organizational_header(line):
        ...
        next_line = next(
            (line for line in lines[i + 1 :] if line.strip()), ""
        ...
        is_header = line.isupper() and (
            ...
            or (
                ...
                and (
                    ...
                    or "street" in next_line.lower()
                    or "avenue" in next_line.lower()
                )
            )
        )
    ...
    for unit in organizational_units:
        ...
        if len(unit) > 2 * max_chunk_size:
            ...
            print(
                f"Warning: Split oversized unit of size {len(unit)} into {len(split_chunks)} chunks"
            )
            continue

        # If this single unit is larger than max_chunk_size, we have to keep it together
...
def process_departments_file(
    ...
    """
    Takes a file containing text from a Congressional Directory and extracts
    department/cabinet/office of the president, member names, and roles using OpenAI's API.
    The extracted information is saved in JSON format.
    ...
    all_data = {"cabinet": [], "executive_office_of_president": [], "departments": []}
        ...
        try:
            ...
            try:
                ...
                if "executive_office_of_president" in chunk_data:
                    all_data["executive_office_of_president"].extend(
                        chunk_data["executive_office_of_president"]
                    )
...
def process_all_departments_files(
        congress_number: int,
        client: openai.OpenAI,
        base_directory: str = "."
    ) -> None:
    """
    Process all DEPARTMENTS files for a given Congress number using the function
    `process_departments_file`.

     Args:
        congress_number: Congress number to process
        client: OpenAI API client
        base_directory: Base directory for the input and output
    """
...
if __name__ == "__main__":
    ...
    process_all_departments_files(congress_number=117, client=input_client)
