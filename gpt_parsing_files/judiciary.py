"""
Extract judiciary courts information from congressional directory files using OpenAI's API.
Extracts court names, circuits/divisions, and all personnel information from the text.
    ...
    try:
        response = client.chat.completions.create(
            ...
            response_format={
                ...
                "json_schema": {
                    "name": "_",
                    "schema": CourtsJsonSchema.model_json_schema(),
                },
            },
            temperature=0.3,
...
def chunk_judiciary_text(text: str, max_chunk_size: int = 15000) -> list[str]:
    """
    ...
    Args:
        text: The full judiciary text
        max_chunk_size: Maximum size for each chunk

    Returns:
        chunks: text chunks, each containing complete court entries
    """
    chunks = []
    lines = text.split("\n")

    def is_major_court_header(line):
        ...
        is_caps = line.isupper() and any(
            indicator in line for indicator in major_court_indicators
        )
...
def process_courts_file(
    ...
    """
    Process the judiciary courts information from a given file. Chunk the file into manageable sizes
    using `chunk_judiciary_text` and extract the courts information using
    `extract_judiciary_courts_info`. Save the processed JSON output to the output directory.

    Args:
        file_name: Name of the file to process
        input_dir: Input directory containing the file
        output_dir: Output directory to save the processed JSON file
        client: OpenAI client
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                ...
                try:
                    ...
                    print(
                        f"JSON Decode Error in chunk {i+1} of file {file_name}: {str(e)}"
                    )
...
def process_all_courts_files(
        congress_number: int,
        client: openai.OpenAI,
        base_directory: str = "."
    ) -> bool:
    """
    Process all JUDICIARY files for a given Congress number by running `process_courts_file` on each
    file
    in the given congress directory. Saves the processed JSON output to the output directory.
    ...
    if not os.path.exists(input_directory):
        ...
        if os.environ.get("GITHUB_ACTIONS") == "true":
            dummy_output_file = os.path.join(
                output_directory, "CDIR-2009-01-01-JUDICIARY.txt_output.json"
            )
            test_data = {
                "courts": [
                    {
    ...
    judiciary_files = [
        ...
        if "JUDICIARY" in f and f.endswith(".txt")
    ]
...
if __name__ == "__main__":
    ...
    process_all_courts_files(congress_number=117, client=input_client)
