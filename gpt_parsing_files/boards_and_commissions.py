...
class boards_and_commissions_json_schema(BaseModel):
    government_bodies: list[GovernmentBody]

def extract_boards_and_commissions_info(text_chunk, client):
    """
    Uses OpenAI's API to extract government bodies (boards and commissions), member names, and roles from the given text.
        ...
        response = client.chat.completions.create(
            ...
            response_format={
                ...
                'json_schema': 
                    {
                        "name":"_", 
                        "schema": boards_and_commissions_json_schema.model_json_schema()
                    }
            },  
            temperature=0.3,
            # max_tokens=10000,
...
def chunk_boards_and_commissions_text(text, max_chunk_size=15000):
    chunks = []
    lines = text.split('\n')
    ...
    def is_government_body_header(line, next_line=""):
        ...
        indicators = [
            'BOARD', 'COMMISSION', 'GROUP', 'COMMITTEE', 
            'PARLIAMENTARY', 'OFFICE', 'FOUNDATION',
            'INSTITUTE', 'ASSOCIATION', 'SENATE', 'HOUSE'
        ]
        
        has_indicator = any(ind in line for ind in indicators)
        
        # Check if followed by contact info
        has_contact = ('phone' in next_line.lower() or 
                      'address' in next_line.lower() or
...
def process_boards_and_commissions_file(file_name, input_dir, output_dir, client):
    # Read file content
    ...
    for chunk_num, chunk in enumerate(chunks, 1):
        ...
        try:
            ...
            boards_and_commissions_info = extract_boards_and_commissions_info(chunk, client)
            ...
            try:
                ...
                if "government_bodies" in chunk_data:
                    all_government_bodies["government_bodies"].extend(chunk_data["government_bodies"])
...
def process_boards_and_commissions_files_for_congress(congress_num, client, base_directory):
    """
    Process boards and commissions files for a specific congress session.

    Args:
        congress_num: int
            Congress session number (e.g., 117)
    """
    print(f"Processing boards and commissions files for Congress {congress_num}...")

    for filename in os.listdir(f'{base_directory}/congressional_directory_files/congress_{congress_num}/txt'):
...
if __name__ == "__main__":
    ...
    # print("Processing boards and commissions file...")
    # process_boards_and_commissions_file('CDIR-2022-10-26-BOARDSANDCOMMISSIONS.txt', 'congressional_directory_files/congress_117/txt', 'outputs/117')

    # print("Processing boards and commissions file...")
    # process_boards_and_commissions_file('CDIR-2022-10-26-BOARDSANDCOMMISSIONS-2.txt', 'congressional_directory_files/congress_117/txt', 'outputs/117')
    process_boards_and_commissions_files_for_congress(117)
