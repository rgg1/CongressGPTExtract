"""
Orchestrator for running multiple processors on congressional data. Allows for
running specific processors for a range of congress sessions, so that you can parse
specific file types for specific congresses, all using the OpenAI API.
"""
...
from international_organizations import (
    process_international_organizations_file_for_congress,
)
...
class ProcessingOrchestrator:
    ...
    def __init__(self, api_key: str = None):
        ...
        if not self.api_key:
            raise ValueError(
                "OpenAI API key not found. Please provide it via command line argument or .env file."
            )
        ...
        self.processors = {
            "judiciary": lambda congress: process_all_courts_files(
                congress, self.client, base_directory
            ),
            "international_organizations": lambda congress: process_international_organizations_file_for_congress(
                congress, self.client, base_directory
            ),
            "independent_agencies": lambda congress: process_independent_agency_file_for_congress(
                congress, self.client, base_directory
            ),
            "house_senate_committees": lambda congress: process_all_committees_for_congress(
                congress, self.client, base_directory
            ),
            "diplomatic_offices": lambda congress: process_diplomatic_offices_file_for_congress(
                congress, self.client, base_directory
            ),
            "departments": lambda congress: process_all_departments_files(
                congress, self.client, base_directory
            ),
            "boards_and_commissions": lambda congress: process_boards_and_commissions_files_for_congress(
                congress, self.client, base_directory
            ),
        }

    def run_processors(
        self, congress_range: List[str], selected_processors: Optional[List[str]] = None
    ) -> None:
        """
        ...
        Raises:
            ValueError: If any processor names are invalid.
        """
        ...
        for congress in congress_range:
            ...
            for processor_name in processors_to_run:
                ...
                try:
                    ...
                    print(
                        f"Error in {processor_name} processor for {congress}th Congress: {str(e)}"
                    )
...
def parse_congress_range(range_str: str) -> List[str]:
    ...
    Args:
        range_str: Represents a single congress or a range of congresses.

    Returns: List of congress numbers as strings.
    """
    if "-" in range_str:
...
def main():
    """
    Main function for running the ProcessingOrchestrator. When called from the command line,
    this function parses command line arguments and runs the orchestrator with the specified
    congress range and processors.
    """
    parser = argparse.ArgumentParser(
        description="Process congressional data across multiple processors"
    )
    parser.add_argument(
        "--congress",
        required=True,
    ...
    parser.add_argument(
        "--api_key", help="OpenAI API key. If not provided, will check the .env file."
    )
