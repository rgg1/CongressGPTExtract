...
def extract_diplomatic_offices_info(text_chunk, client):
    """
    Uses OpenAI's API to extract diplomatic offices, representatives, and their roles from the given text.
...
def chunk_diplomatic_offices_text(text, max_chunk_size=15000):
    """
    ...
    Args:
        text (str): The full diplomatic offices text
        max_chunk_size (int): Maximum size for each chunk
        
    Returns:
        list[str]: List of text chunks, each containing complete office entries
    """
    chunks = []
    lines = text.split('\n')
    ...
    def is_diplomatic_office_header(line, next_line=""):
        ...
        has_embassy_info = any(word in next_line.lower() for word in ['embassy', 'delegation', 'mission', 'apostolic', 'consular'])
        
        # Exclude page headers and section titles
        exclude_headers = ['CONGRESSIONAL DIRECTORY', 'FOREIGN DIPLOMATIC OFFICES', 'Page']
...
def process_diplomatic_offices_file(file_name, input_dir, output_dir, client):
    """
    Process a diplomatic offices file and extract diplomatic representatives.
    ...
    Args:
        ...
        client: OpenAI
            OpenAI client to use for processing
            ...
            try:
                ...
                try:
                    ...
                    if "diplomatic_representatives" in diplomatic_offices_data:
                        all_diplomatic_representatives["diplomatic_representatives"].extend(diplomatic_offices_data["diplomatic_representatives"])
...
def process_diplomatic_offices_file_for_congress(congress_num, client, base_directory):
    """
    Process the diplomatic offices file for a specific congress session.

    Args:
        congress_num (str): The congress session to process (e.g., "117")
    ...
    input_directory = os.path.join(base_directory, "congressional_directory_files", f"congress_{congress_num}", "txt")
    ...
    if not os.path.exists(input_directory):
        ...
        if os.environ.get("GITHUB_ACTIONS") == "true":
            dummy_output_file = os.path.join(output_directory, "CDIR-2022-10-26-DIPLOMATICOFFICES.txt_output.json")
            test_data = {
                "diplomatic_representatives": [
                    {
                        "name": "Jane Smith",
    ...
    diplomatic_files = [f for f in os.listdir(input_directory) if 'DIPLOMATICOFFICES' in f and f.endswith('.txt')]
...
if __name__ == "__main__":
    ...
    # process_diplomatic_offices_file('CDIR-2022-10-26-DIPLOMATICOFFICES.txt', 'congressional_directory_files/congress_117/txt', 'outputs/117')
    process_diplomatic_offices_file_for_congress(117, client)
