...
def clean_committee_name(name):
    ...
    Args:
        name (str): The committee name to clean.

    Returns:
        str: The cleaned committee name
    """
    # lowercase
    name = name.lower()
...
def load_committee_mappings(csv_file):
    """
    Load committee names and their Thomas IDs from CSV file with additional variations
    that can be used for matching.

    Args:
        csv_file (str): The path to the CSV file containing committee mappings.
    
    Returns:
        Tuple[Dict[str, str], Dict[str, Set[str]]]: A dictionary mapping committee names to Thomas IDs
        and a dictionary mapping original names to cleaned variations for validation.
    """
    mappings = {}
    ...
    with open(csv_file, 'r') as f:
        ...
        for row in reader:
            ...
            variations = {
                ...
                clean_committee_name(original_name.replace("House Committee on ", "").replace("Senate Committee on ", "")),
                clean_committee_name(original_name.split("Committee")[-1] if "Committee" in original_name else original_name)
            }
...
def update_committees_with_thomas_ids(data, mappings):
    """
    Update committee data with Thomas IDs based on the provided mappings.

    Args:
        data (Dict): The committee data to update.
        mappings (Dict): The dictionary mapping committee names to Thomas IDs.

    Returns:
        Tuple[Dict, List, List]: The updated committee data, list of unmatched committees, and list of matched
    """
    ...
    for committee in updated_data['committees']:
        ...
        if committee_name in mappings:
            ...
            matched.append(f"Main Committee: {original_name} -> {mappings[committee_name]}")
        else:
            unmatched.append(f"Main Committee: {original_name} (cleaned: {committee_name})")
        
        if 'subcommittees' in committee:
            for subcommittee in committee['subcommittees']:
                subcommittee_name = clean_committee_name(subcommittee['subcommittee_name'])
                ...
                variations = [
                    ...
                    clean_committee_name(original_subname + " " + committee['committee_name'])
                ]
                
                for variation in variations:
                    if variation in mappings:
                        subcommittee['thomas_id'] = mappings[variation]
                        matched.append(f"Subcommittee: {original_subname} -> {mappings[variation]}")
                        found_match = True
                        break
                
                if not found_match:
                    unmatched.append(f"Subcommittee: {original_subname} (cleaned: {subcommittee_name})")
...
def main():
    ...
    json_files = ["outputs/117/" + f for f in os.listdir("outputs/117") if 'HOUSECOMMITTEES.txt' in f]
    ...
    for json_file in json_files:
        ...
        try:
            ...
            updated_data, unmatched, matched = update_committees_with_thomas_ids(committee_data, mappings)
